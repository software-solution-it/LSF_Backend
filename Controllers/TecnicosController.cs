...
namespace LSF.Controllers
{
    ...
    public class TecnicosController : ControllerBase
    {
        ...
        public TecnicosController(APIDbContext dbContext)
        {
        ...
        [HttpGet]
        public IEnumerable<Tecnicos> Get()
        {
            return _dbContext.Tecnicos.ToList();
        }

        // GET api/<TecnicosController>/5
        [HttpGet("{id}")]
        public Tecnicos Get(int id)
        {
            return _dbContext.Tecnicos.FirstOrDefault(t => t.Id == id);
        }

        [HttpPost]
        public async Task<IActionResult> PostTecnico(Tecnicos tecnico)
        {
            if (tecnico == null)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, Tecnicos tecnico)
        {
            var tecnicoExistente = await _dbContext.Tecnicos.FindAsync(id);
        ...
        public IActionResult Delete(int id)
        {
            ...
            var tecnico = _dbContext.Tecnicos.Find(id);
            if (tecnico == null)
            {
                return NotFound("Técnico não encontrado");
            }
